...
A self-hosted cloud control panel I built for deploying and managing Docker containers from a web UI — my own little personal cloud, running on my own machine.

![Yiftach Cloud Platform demo](public/gif/gif-final.gif)

The idea is simple: pick where your service comes from — a ready-made preset, any Docker image, or a GitHub repository — and the platform turns it into a running container you can watch, manage, and inspect from the browser. Everything runs locally on Windows, with the Docker daemon living inside WSL2.

The project is split into three standalone packages: a platform backend (the REST API), a build agents microservice (a headless worker that does the image builds), and a React frontend.
...
- **My Services** — all your containers in one table with live stats. Start, stop, and delete them, or open a service to see its details, resource usage, and logs (with live follow and log clearing).

- **New Service wizard** — three ways to create a service: a managed preset (Postgres, Redis, and friends, with sensible default ports), any image from a registry, or a GitHub repository.
...
- **Build from GitHub** — paste a repo URL (optionally with `#branch` or `#tag`), and a build agent clones it, builds the image with BuildKit, and streams the build log live to your browser. When the build finishes, the container is created and started automatically. If you didn't specify ports, the platform publishes the ports the image EXPOSEs on its own.
...
- **Build Agents microservice** — the builds run in a separate worker service, not in the API server. Builds are contained: repos are cloned safely inside a container instead of on the host as root, so untrusted code never touches the machine directly. The Build Agents page shows every agent live via heartbeats — idle, building, or offline, with uptime and last-seen time.

- **Self-healing Docker daemon** — WSL doesn't auto-start, and the platform doesn't care. When a request finds the daemon down, the backend boots the WSL distro by itself, retries, and holds the distro open for as long as the server runs.
...
- **`builder-service-backend/`** — the build agents microservice. It has no HTTP server at all; it just polls the platform: claim a job → shallow-clone the repo → build the image with BuildKit → resolve the ports → create the container through the platform API → report the result. It sends heartbeats the whole time so the UI knows it's alive.

- **`frontend/`** — a React single-page app that only ever talks to the platform API.

So a GitHub build flows like this: the wizard posts one request with the repo URL and the container config. The platform queues it. An idle build agent claims it, clones the repo, builds the image (streaming progress lines back so the browser shows a live log), then asks the platform to create and start the container. The wizard just watches the job status and jumps to My Services when it's done.
